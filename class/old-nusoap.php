...
'apos' => "'",
     ...
     */
    public function serializeEnvelope($body, $headers = false, $namespaces = [], $style = 'rpc')
    {
...
class XMLSchema extends nusoap_base
{
    ...
    public function serializeSchema()
    {
        ...
        foreach ($this->complexTypes as $typeName => $attrs) {
            ...
            if (isset($attrs['compositor']) && 'all' == $attrs['compositor']) {
            ...
            if (isset($attrs['compositor']) && '' != $attrs['compositor'] && 'all' != $attrs['compositor']) {
                $contentStr = "<$schemaPrefix:$attrs[compositor]>" . $contentStr . "</$schemaPrefix:$attrs[compositor]>";
            }
...
class soap_transport_http extends nusoap_base
{
     ...
     * @param    int $timeout set timeout in seconds
     * @return	string data
     */
    public function send($data, $timeout = 0)
    {
        ...
        if ('' != $this->proxyhost && '' != $this->proxyport) {
            $this->outgoing_payload = "$this->request_method $this->url " . mb_strtoupper($this->scheme) . "/$this->protocol_version\r\n";
        } else {
            $this->outgoing_payload = "$this->request_method $this->path " . mb_strtoupper($this->scheme) . "/$this->protocol_version\r\n";
        }
        ...
        $this->debug('received ' . mb_strlen($this->incoming_payload) . ' bytes of data from server');
        ...
        $this->debug('received incoming payload: ' . mb_strlen($this->incoming_payload));
        ...
        if (isset($headers['Content-Encoding']) && '' != $headers['Content-Encoding']) {
            if ('deflate' == $headers['Content-Encoding'] || 'gzip' == $headers['Content-Encoding']) {
                // if decoding works, use it. else assume data wasn't gzencoded

                if (function_exists('gzinflate')) {
                    //$timer->setMarker('starting decoding of gzip/deflated content');

                    if ('deflate' == $headers['Content-Encoding'] && $degzdata = @gzinflate($data)) {
                        $data = $degzdata;
                    } elseif ('gzip' == $headers['Content-Encoding'] && $degzdata = gzinflate(mb_substr($data, 10))) {
                        $data = $degzdata;
                    } else {
     ...
     * @return	string data
     */
    public function sendHTTPS($data, $timeout = 0)
    {
        //global $t;

        //$t->setMarker('inside sendHTTPS()');

        $this->debug('entered sendHTTPS() with data of length: ' . mb_strlen($data));
     ...
     * @return string
     */
    public function decodeChunked($buffer)
    {
        // length := 0
...
class soap_server extends nusoap_base
{
    ...
    public $responseHeaders = false;

    public $headers = '';

    public $request = '';

    public $charset_encoding = 'UTF-8';
     ...
     *
     * @param bool $wsdl path or URL to a WSDL file
     */
    ...
    public function service($data)
    {
        ...
        if ('' == $data && $this->wsdl) {
            ...
            if (isset($this->debug_flag) && 1 == $this->debug_flag) {
        ...
        if ($this->verify_method($this->methodname, $request_data)) {
            ...
            if ($request_data) {
                ...
                if (!function_exists('call_user_func_array')) {
                    ...
                    $method_response = call_user_func_array((string)$this->methodname, $request_data);
                }
            ...
            if (isset($method_response) && '' != $method_response || is_bool($method_response)) {
                ...
                if ('soap_fault' == get_class($method_response)) {
                ...
                } elseif ('soapval' == get_class($method_response)) {
     ...
     * @param bool  $style (rpc|literal)
     * @param mixed $use
     * @return bool
     */
    public function register($name, $in = false, $out = false, $namespace = false, $soapaction = false, $style = false, $use = false)
    {
        if ($this->externalWSDLURL) {
...
'style' => $style,
     ...
     */
    public function fault($faultcode, $faultactor, $faultstring = '', $faultdetail = '')
    {
     ...
     */
    public function configureWSDL($serviceName, $namespace = false, $endpoint = false, $style = 'rpc', $transport = 'http://schemas.xmlsoap.org/soap/http')
    {
...
class wsdl extends XMLSchema
{
     ...
     */
    public function __construct($wsdl = '', $proxyhost = false, $proxyport = false)
    {
        $this->wsdl = $wsdl;
    ...
    {
        if ('schema' == $this->status || preg_match('schema$', $name)) {
     ...
     */
    public function serializeType($name, $type, $value, $use = 'encoded')
    {
        ...
        if (mb_strpos($type, ':')) {
            ...
            if ($ns == $this->XMLSchemaVersion ||
                ...
                if ($this->charencoding && 'string' == $uqType && 'string' == gettype($value)) {
...
class soap_parser extends nusoap_base
{
     ...
     * @param mixed $method
     */
    public function __construct($xml, $encoding = 'UTF-8', $method = '')
    {
        $this->xml = $xml;
            ...
            // build complex values
                ...
                if (is_numeric($this->message[$pos]['cdata'])) {
                    if (mb_strpos($this->message[$pos]['cdata'], '.')) {
                        $this->message[$pos]['result'] = (float)$this->message[$pos]['cdata'];
                    } else {
                        $this->message[$pos]['result'] = (int)$this->message[$pos]['cdata'];
                    }
                } else {
...
class soapclient extends nusoap_base
{
     ...
     * @param null   $rpcParams  optional treat params as RPC for use="literal"
     *                           This can be used on a per-call basis to overrider defaultRpcParams.
     * @return    mixed
     */
    public function call($operation, $params = [], $namespace = '', $soapAction = '', $headers = false, $rpcParams = null)
    {
        ...
        if ('wsdl' == $this->endpointType && $opData = $this->getOperationData($operation)) {
            ...
            $namespace = $opData['input']['namespace'] ?? 'http://testuri.org';
            ...
            $this->debug('payload size: ' . mb_strlen($payload));
        ...
        $this->debug('sending msg (len: ' . mb_strlen($soapmsg) . ") w/ soapaction '$soapAction'...");
     ...
     */
    public function send($msg, $soapaction = '', $timeout = 0)
    {
        // detect transport
        ...
        switch (true) {
            ...
            case preg_match('^http', $this->endpoint):
                ...
                if ($err = $http->getError()) {
                    ...
                    $this->debug('got response, length: ' . mb_strlen($response));
     ...
     * @param    string $proxyport
     */
    public function setHTTPProxy($proxyhost, $proxyport)
    {
        $this->proxyhost = $proxyhost;
