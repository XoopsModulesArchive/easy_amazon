...
/*
$Id: new-nusoap.php,v 1.1 2006/03/27 15:23:16 mikhail Exp $
 ...
 * @version  $Id: new-nusoap.php,v 1.1 2006/03/27 15:23:16 mikhail Exp $
 */
class nusoap_base
{
    public $title = 'NuSOAP';
    ...
    public $revision = '$Revision: 1.1 $';

    public $error_str = false;
...
'apos' => "'",
    ...
    public function serialize_val($val, $name = false, $type = false, $name_ns = false, $type_ns = false, $attributes = false, $use = 'encoded')
    {
        ...
        if ($name_ns) {
            $prefix = 'nu' . random_int(1000, 9999);
        ...
        if ('' != $type_ns && $type_ns == $this->namespaces['xsd']) {
            ...
            $type_prefix = 'ns' . random_int(1000, 9999);
                    ...
                    if (is_array($val) && count($val) > 0) {
                        ...
                        if (count($array_types) > 1) {
                            ...
                            if ('' != $tt_ns && $tt_ns == $this->namespaces['xsd']) {
                                ...
                                $tt_prefix = 'ns' . random_int(1000, 9999);
     ...
     */
    public function serializeEnvelope($body, $headers = false, $namespaces = [], $style = 'rpc', $use = 'encoded')
    {
    ...
    {
        return $this->namespaces[$prefix] ?? false;
    }
...
class XMLSchema extends nusoap_base
{
    ...
    public $currentComplexType = false;

    public $elements = [];

    public $currentElement = false;

    public $simpleTypes = [];

    public $currentSimpleType = false;
    ...
    public function parseString($xml, $type)
    {
        ...
        if ('' != $xml) {
            ...
            if ('schema' == $type) {
                xml_set_elementHandler($this->parser, 'schemaStartElement', 'schemaEndElement');

                xml_set_character_dataHandler($this->parser, 'schemaCharacterData');
            } elseif ('xml' == $type) {
                xml_set_elementHandler($this->parser, 'xmlStartElement', 'xmlEndElement');

                xml_set_character_dataHandler($this->parser, 'xmlCharacterData');
            }
        ...
        if (count($attrs) > 0) {
            foreach ($attrs as $k => $v) {
                ...
                if (preg_match('^xmlns', $k)) {
                    ...
                    if ($ns_prefix = mb_substr(mb_strrchr($k, ':'), 1)) {
                    ...
                    if ('http://www.w3.org/2001/XMLSchema' == $v || 'http://www.w3.org/1999/XMLSchema' == $v) {
...
'arrayType' => $arrayType,
     ...
     * @see xmlschema
     */
    public function addSimpleType($name, $restrictionBase = '', $typeClass = 'simpleType', $phpType = 'scalar')
    {
...
class soapval extends nusoap_base
{
     ...
     * @param bool   $type       optional type name
     * @param mixed  $value      optional value
     * @param mixed  $element_ns
     * @param mixed  $type_ns
     * @param bool   $attributes associative array of attributes to add to element serialization
     */
...
class soap_transport_http extends nusoap_base
{
    ...
    public function __construct($url)
    {
        ...
        if (isset($u['user']) && '' != $u['user']) {
            $this->setCredentials($u['user'], $u['pass'] ?? '');
        }
    }
     ...
     * @return	string data
     */
    public function send($data, $timeout = 0, $response_timeout = 30)
    {
     ...
     * @return	string data
     */
    public function sendHTTPS($data, $timeout = 0, $response_timeout = 30)
    {
     ...
     */
    public function setCredentials($username, $password, $authtype = 'basic', $digestRequest = [])
    {
        global $_SERVER;
        ...
        if (isset($this->outgoing_headers['Authorization'])) {
            $this->debug('Authorization header set: ' . mb_substr($this->outgoing_headers['Authorization'], 0, 12) . '...');
        } else {
...
//		}
            ...
            if ('https' == $this->scheme) {
                ...
                foreach ($header_array as $header_line) {
                    ...
                    if (count($arr) > 1) {
                        $this->incoming_headers[mb_strtolower(trim($arr[0]))] = trim($arr[1]);
                    }
                }
            }
        }
        ...
        if (isset($this->incoming_headers['www-authenticate']) && mb_strstr($header_array[0], '401 Unauthorized')) {
            $this->debug('Got 401 Unauthorized with WWW-Authenticate: ' . $this->incoming_headers['www-authenticate']);

            if (mb_substr('Digest ', $this->incoming_headers['www-authenticate'])) {
        ...
        if (isset($this->incoming_headers['content-encoding']) && '' != $this->incoming_headers['content-encoding']) {
            if ('deflate' == mb_strtolower($this->incoming_headers['content-encoding']) || 'gzip' == mb_strtolower($this->incoming_headers['content-encoding'])) {
                ...
                if (function_exists('gzuncompress')) {
                    ...
                    if ('deflate' == $this->incoming_headers['content-encoding'] && $degzdata = @gzuncompress($data)) {
                        $data = $degzdata;
                    } elseif ('gzip' == $this->incoming_headers['content-encoding'] && $degzdata = gzinflate(mb_substr($data, 10))) {	// do our best
                        $data = $degzdata;
                    } else {
...
{
    public $headers = [];			// HTTP headers of request
    public $request = '';				// HTTP request
    public $requestHeaders = '';		// SOAP headers from request (incomplete namespace resolution) (text)
    public $document = '';				// SOAP body request portion (incomplete namespace resolution) (text)
    public $requestSOAP = '';			// SOAP payload for request (text)
    public $methodURI = '';			// requested method namespace URI
    public $methodname = '';			// name of method requested
    public $methodparams = [];	// method parameters from request
    public $xml_encoding = '';			// character set encoding of incoming (request) messages
    public $SOAPAction = '';			// SOAP Action from request

    public $outgoing_headers = []; // HTTP headers of response
    public $response = '';				// HTTP response
    public $responseHeaders = '';		// SOAP headers for response (text)
    public $responseSOAP = '';			// SOAP payload for response (text)
    public $methodreturn = false;		// method return to place in response
    public $methodreturnisliteralxml = false;	// whether $methodreturn is a string of literal XML
    public $fault = false;				// SOAP fault for response
    public $result = 'successful';		// text indication of result (for debugging)

    public $operations = [];		// assoc array of operations => opData
    public $wsdl = false;				// wsdl instance
    public $externalWSDLURL = false;	// URL for WSDL
    public $debug_flag = false;		// whether to append debug to response as XML comment
    ...
    public function parse_http_headers()
    {
        ...
        if (function_exists('getallheaders')) {
            ...
            if (mb_strpos($this->headers['Content-Type'], '=')) {
            ...
            foreach ($_SERVER as $k => $v) {
                if ('HTTP_' == mb_substr($k, 0, 5)) {
                    $k = str_replace(' ', '-', ucwords(mb_strtolower(str_replace('_', ' ', mb_substr($k, 5)))));
                } else {
                    $k = str_replace(' ', '-', ucwords(mb_strtolower(str_replace('_', ' ', $k))));
                }
    ...
    public function invoke_method()
    {
        ...
        if ($this->methodparams) {
            ...
            if (!function_exists('call_user_func_array')) {
                ...
                $this->methodreturn = call_user_func_array((string)$this->methodname, $this->methodparams);
            }
        } else {
    ...
    public function serialize_return()
    {
        ...
        if (isset($this->methodreturn) && ('' != $this->methodreturn || is_bool($this->methodreturn))) {
            ...
            if ('soap_fault' == get_class($this->methodreturn)) {
    ...
    public function send_response()
    {
        ...
        if (mb_strlen($payload) > 1024 && isset($this->headers) && isset($this->headers['Accept-Encoding'])) {
            if (mb_strstr($this->headers['Accept-Encoding'], 'deflate')) {
     ...
     * @param bool   $soapaction
     * @param bool   $style         optional (rpc|document)
     * @param bool   $use           optional (encoded|literal)
     * @param string $documentation optional Description to include in WSDL
     * @return bool
     */
    public function register($name, $in = false, $out = false, $namespace = false, $soapaction = false, $style = false, $use = false, $documentation = '')
    {
        if ($this->externalWSDLURL) {
        ...
        if (false === $soapaction) {
            $SERVER_NAME = $_SERVER['SERVER_NAME'] ?? $GLOBALS['SERVER_NAME'];

            $SCRIPT_NAME = $_SERVER['SCRIPT_NAME'] ?? $GLOBALS['SCRIPT_NAME'];
...
'style' => $style,
    ...
    public function webDescription()
    {
		...
		<div class=content>
			...
			<div class=nav>
				<p>View the <a href="' . ($GLOBALS['PHP_SELF'] ?? $_SERVER['PHP_SELF']) . '?wsdl">WSDL</a> for the service.
     ...
     * @param bool   $namespace             optional tns namespace
     * @param bool   $endpoint              optional URL of service endpoint
     * @param string $style                 optional (rpc|document) WSDL style (also specified by operation)
     * @param string $transport             optional SOAP transport
     * @param bool   $schemaTargetNamespace optional targetNamespace for service schema
     */
    public function configureWSDL($serviceName, $namespace = false, $endpoint = false, $style = 'rpc', $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = false)
    {
        ...
        $SERVER_PORT = $_SERVER['SERVER_PORT'] ?? $GLOBALS['SERVER_PORT'];
...
class wsdl extends nusoap_base
{
     ...
     */
    public function __construct($wsdl = '', $proxyhost = false, $proxyport = false, $proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30)
    {
        $this->wsdl = $wsdl;
                                ...
                                if ('' != $url) {
                                    ...
                                    if (!isset($urlparts['host'])) {
                                        $url = $wsdlparts['scheme'] . '://' . $wsdlparts['host'] .
                                                    mb_substr($wsdlparts['path'], 0, mb_strrpos($wsdlparts['path'], '/') + 1) . $urlparts['path'];
                                    }
            ...
            foreach ($this->bindings as $bindingName => $attrs) {
                ...
                foreach ($attrs['operations'] as $opName => $opParts) {
                    ...
                    if (isset($opParts['documentation']) && '' != $opParts['documentation']) {
                        $portType_xml .= '<documentation>' . htmlspecialchars($opParts['documentation'], ENT_QUOTES | ENT_HTML5) . '</documentation>';
                    }
     ...
     */
    public function serializeRPCParameters($operation, $direction, $parameters)
    {
     ...
     * @param bool   $encodingStyle , use to add encoding changes to serialisation
     * @return string serialization
     */
    public function serializeType($name, $type, $value, $use = 'encoded', $encodingStyle = false)
    {
        ...
        if (mb_strpos($type, ':')) {
            ...
            if ($ns == $this->XMLSchemaVersion) {
                ...
                if ('boolean' == $uqType && !$value) {
                    $value = 'false';
                } elseif ('boolean' == $uqType) {
                    $value = 'true';
                }
                ...
                if ('http://xml.apache.org/xml-soap' == $ns) {
                    if ('Map' == $uqType) {
                        $contents = '';
            ...
            if (isset($typeDef['elements']) && is_array($typeDef['elements'])) {
                ...
                foreach ($typeDef['elements'] as $eName => $attrs) {
                    ...
                    if (isset($optionals) && !isset($xvalue[$eName])) {
                        ...
                        if (isset($attrs['maxOccurs']) && 'unbounded' == $attrs['maxOccurs'] && isset($v) && is_array($v) && 'arraySimple' == $this->isArraySimpleOrStruct($v)) {
                            $vv = $v;
     ...
     * @return bool
     */
    public function addOperation($name, $in = false, $out = false, $namespace = false, $soapaction = false, $style = 'rpc', $use = 'encoded', $documentation = '')
    {
        if ('rpc' == $style && 'encoded' == $use) {
            $encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/';
        } else {
...
class soap_parser extends nusoap_base
{
    ...
    public function decodeSimple($value, $type, $typens)
    {
        ...
        if ('nonPositiveInteger' == $type || 'negativeInteger' == $type
            || 'nonNegativeInteger' == $type || 'positiveInteger' == $type || 'unsignedInt' == $type
    ...
    public function buildVal($pos)
    {
        ...
        if (mb_strpos($this->message[$pos]['cdata'], '&')) {
...
class soapclient extends nusoap_base
{
     ...
     * @param bool   $headers    optional array of soapval objects for headers
     * @param null   $rpcParams  optional no longer used
     * @param string $style      optional (rpc|document) the style to use when serializing parameters (WSDL can override)
     * @param string $use        optional (encoded|literal) the use when serializing parameters (WSDL can override)
     * @return    mixed
     */
    public function call($operation, $params = [], $namespace = '', $soapAction = '', $headers = false, $rpcParams = null, $style = 'rpc', $use = 'encoded')
    {
        ...
        if ('wsdl' == $this->endpointType && $opData = $this->getOperationData($operation)) {
            ...
            $namespace = $opData['input']['namespace'] ?? ('' != $namespace ? $namespace : 'http://testuri.org');
        ...
        $this->debug('SOAP message length: ' . mb_strlen($soapmsg) . ' contents: ' . mb_substr($soapmsg, 0, 1000));
    ...
    {
        $this->debug('Entering parseResponse() for data of length ' . mb_strlen($data) . ' and type ' . $headers['content-type']);
     ...
     */
    public function setCredentials($username, $password, $authtype = 'basic')
    {
        $this->username = $username;
        ...
        foreach ($this->operations as $operation => $opData) {
            if ('' != $operation) {
                ...
                if (count($opData['input']['parts']) > 0) {
                    ...
                    $paramStr = mb_substr($paramStr, 0, -1);
                }
